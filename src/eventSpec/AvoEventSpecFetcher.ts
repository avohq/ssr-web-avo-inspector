 ...
 *
 * Uses global fetch (available in browsers, Web Workers, Node.js 18+).
...
export class AvoEventSpecFetcher {
  ...
  /**
   * Makes an HTTP GET request using global fetch (available in browsers, workers, Node 18+).
   * Returns the parsed JSON response or null on failure.
   *
   * Uses AbortController + clearTimeout for per-request timeout with cleanup.
   */
