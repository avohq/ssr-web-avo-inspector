 ...
 * - Global sweep: Runs every 50 cache operations (get AND set)
...
export class EventSpecCache {
  ...
  /** Global cache operation counter to track when to sweep (increments on BOTH get AND set) */
  ...
  get(
    ...
    entry.eventCount++;
    // Increment global counter on GET (both get and set increment)
  ...
  set(
    ...
    // Increment global counter on SET (both get and set increment)
    this.globalEventCount++;
