...
describe("Sessions", () => {
  ...
  test("Session started on trackSchemaFromEvent", async () => {
    // Given
  ...
  test("Delays session init if items from last sessions are not loaded", async () => {
    // Given
    const callMoment = Date.now();

    storage.removeItem(AvoSessionTracker.lastSessionTimestampKey);
    let _ = new AvoInspector(defaultOptions);
