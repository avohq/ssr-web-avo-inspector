...
describe("EventSpecCache", () => {
  ...
  test("global sweep triggered by set operations", () => {
    const now = Date.now();
    const dateSpy = jest.spyOn(Date, "now");
    ...
    // Add 49 more entries (set operations increment globalEventCount)
    for (let i = 0; i < 49; i++) {
      cache.set("key", "stream", `event-${i}`, makeSpec());
    }

    // old-event should still be in cache (not yet swept)
    expect(cache.size()).toBeGreaterThanOrEqual(1);

    // The 50th set should trigger sweep
    cache.set("key", "stream", "event-50", makeSpec());

    // old-event should be swept (age > 60s)
    const result = cache.get("key", "stream", "old-event");
    expect(result).toBeUndefined();
